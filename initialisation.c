...
void data_init(t_philo *philosophers, t_data *data) // initialiser les variables de la structure data
{
...
void chopsticks_init(pthread_mutex_t *chopsticks, int number_of_philosophers) // initialiser les chopsticks toujours recréer la variable, et recréer ce dont on a besoin dans la structure  
{
    int i;
...
void input_init(char **argv, t_philo *philosophers) //synchroniser les time to 'action' avec les arguments entrés
{
...
void    philosophers_init(char **argv, t_philo *philosophers, t_data *data, pthread_mutex_t *chopsticks) //!init des paramètres du philo et set des chopsticks dans le sens du cercle
{
    int i;
