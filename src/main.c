...
int	input_checker_2(char **argv, int i)
{
		...
		|| (i < 5 && ft_atoi(argv[i]) < 0))
	{
		...
		{
			printf("Philosophers are not allowed to eat any meal.\n");
			return (-1);
		}
		else
		{
...
int	input_checker(int argc, char **argv)
{
	...
	if (i < 5)
	{
		printf("You should enter the number of philosphers,"
			"the time to die, the time to eat,"
			"time to sleep and the number of loop.\n");
		return (-1);
	}
	return (i);
}
...
int	main(int argc, char **argv)
{
	...
	phinit(argv, philosophers, &data, chopsticks); //! initialiser les philos
